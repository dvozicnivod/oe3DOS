...
subplot(3,1,1);
plot(t1,x1), axis('xy'), ylabel('amplituda [V]'), xlabel (' vreme [s]' ), title('N1 = 32');

x2 = cos(2*pi*f1*t2) + 0.5*cos(2*pi*f2*t2)+3*cos(2*pi*f3*t2);
subplot(3,1,2);
plot(t2,x2), axis('xy'), ylabel('amplituda [V]'), xlabel (' vreme [s]' ), title('N2 = 128');

x3 = cos(2*pi*f1*t3) + 0.5*cos(2*pi*f2*t3)+3*cos(2*pi*f3*t3);
subplot(3,1,3);
plot(t3,x3), axis('xy'), ylabel('amplituda [V]'), xlabel (' vreme [s]' ), title('N3 = 1024');
...
[pks1, locs1] = findpeaks(abs(X1fft),'NPeaks',3);
[pks2, locs2] = findpeaks(abs(X2fft),'NPeaks',3);
[pks3, locs3] = findpeaks(abs(X3fft),'NPeaks',3);
...
error2 = dominantComp2 - [1,3,7];
error3 = dominantComp3 - [1,3,7];
